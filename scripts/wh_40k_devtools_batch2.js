// wh_40k_devtools_batch2.js — 40K mini photo scraper
// Batch 2/5: units 72–142 of 351 (71 units)
// ============================================================
// WHERE TO PASTE:
...
//   Checkpoints every 5 units. Final: wh_40k_minis_batch2.zip
//
// EXPECTED RUNTIME: ~9 min (71 units × ~8s each).
// RUN ORDER: Run batch1 before this. Wait for it to finish.
//
...
(async () => {
  ...
  // Batch 2/5: 71 entries (units 72–142 of 351).
  // [Legends] and (variant) suffixes stripped from search queries.
  const UNITS = [
  ["wraithblades", "Wraithblades"],
  ["wraithguard", "Wraithguard"],
  ["ynnari-incubi", "Ynnari Incubi"],
  ["ynnari-kabalite-warriors", "Ynnari Kabalite Warriors"],
  ["ynnari-reavers", "Ynnari Reavers"],
  ["ynnari-wyches", "Ynnari Wyches"],
  ["aquila-kill-team", "Aquila Kill Team"],
  ["daemonhost-legends", "Daemonhost"],
  ["damned-legionnaires-legends", "Damned Legionnaires"],
  ["deathwatch-kill-team", "Deathwatch Kill Team"],
  ["deathwatch-terminator-squad-legends", "Deathwatch Terminator Squad"],
  ["exaction-squad", "Exaction Squad"],
  ["fortis-kill-team-legends", "Fortis Kill Team"],
  ["grey-knights-terminator-squad", "Grey Knights Terminator Squad"],
  ["imperial-navy-breachers", "Imperial Navy Breachers"],
  ["indomitor-kill-team-legends", "Indomitor Kill Team"],
  ["inquisitor-ostromandeus-legends", "Inquisitor Ostromandeus"],
  ["inquisitorial-agents", "Inquisitorial Agents"],
  ["kill-team-cassius-legends", "Kill Team Cassius"],
  ["proteus-kill-team-legends", "Proteus Kill Team"],
  ["rogue-trader-entourage", "Rogue Trader Entourage"],
  ["sisters-of-battle-squad", "Sisters of Battle Squad"],
  ["spectrus-kill-team-legends", "Spectrus Kill Team"],
  ["subductor-squad", "Subductor Squad"],
  ["veteran-bike-squad-legends", "Veteran Bike Squad"],
  ...
  ["bullgryn-squad", "Bullgryn Squad"],
  ["cadian-command-squad", "Cadian Command Squad"],
  ["cadian-heavy-weapons-squad", "Cadian Heavy Weapons Squad"],
  ["cadian-recon-squad", "Cadian Recon Squad"],
  ...
  ["gaunts-ghosts", "Gaunt's Ghosts"],
  ["hippogriff-afv", "Hippogriff AFV"],
  ["kasrkin", "Kasrkin"],
  ["krieg-combat-engineers", "Krieg Combat Engineers"],
  ["krieg-command-squad", "Krieg Command Squad"],
  ["krieg-heavy-weapons-squad", "Krieg Heavy Weapons Squad"],
  ...
  ["crusader-squad-legends", "Crusader Squad"],
  ["sternguard-veteran-squad", "Sternguard Veteran Squad"],
  ["sword-brethren-squad", "Sword Brethren Squad"],
  ["death-company-intercessors", "Death Company Intercessors"],
  ["death-company-marines", "Death Company Marines"],
  ["death-company-marines-with-bolt-rifles", "Death Company Marines with Bolt Rifles"],
  ["death-company-marines-with-boltguns-legends", "Death Company Marines with Boltguns"],
  ["death-company-marines-with-boltguns-and-jump-packs-legends", "Death Company Marines with Boltguns and Jump Packs"],
  ["death-company-marines-with-jump-packs", "Death Company Marines with Jump Packs"],
  ["sanguinary-guard", "Sanguinary Guard"],
  ["beasts-of-nurgle", "Beasts of Nurgle"],
  ["bloodcrushers-40k", "Bloodcrushers"],
  ["bloodletters-40k", "Bloodletters"],
  ["blue-horrors", "Blue Horrors"],
  ["daemonettes-40k", "Daemonettes"],
  ["fiends-40k", "Fiends"],
  ["flamers", "Flamers"]
  ];

  console.log('[wh] batch 2/5: units to fetch:', UNITS.length);
  ...
  for (let i = 0; i < UNITS.length; i++) {
    ...
    const label = `[wh] ${i+1}/${UNITS.length} ${slug} (b2/5)`;
    ...
    if ((i + 1) % 5 === 0) {
      console.log(
        `[wh] checkpoint ${i+1}/${UNITS.length} b2 ` +
      ...
      try {
        ...
        triggerDownload(cpBlob,
          `wh_40k_minis_batch2_cp${String(i+1).padStart(2,'0')}.zip`);
  ...
  // Final download
  console.log(
    `[wh] DONE batch 2/5. ok=${ok} ok_canvas=${ok_canvas} ` +
    `no_search_result=${no_search_result} no_product_image=${no_product_image} ` +
    `fail=${fail}`
  );
  const finalBlob = await zip.generateAsync({ type: 'blob' });
  triggerDownload(finalBlob, 'wh_40k_minis_batch2.zip');
  console.log('[wh] wh_40k_minis_batch2.zip download triggered');
