"""
Warhammer 40,000 10th edition army validator.

validate_wh40k(army) -> ValidationResult   (legacy function — delegates to class)
...
@register
class WH40KValidator(BaseValidator):
    code = "w40k10"
    name = "Warhammer 40,000"
    supports_groups = False
    supports_alliances = False
    group_label = "Detachment"
    group_label_plural = "Detachments"
    auxiliary_label = "Auxiliary"
    color = "#4a5d3a"
    default_format = "incursion"
...
def validate_wh40k(army) -> ValidationResult:
    """Standalone function kept for backward compat and direct import."""
    issues = []

    limit = army.points_limit
    cap = WH40KValidator.formats.get(army.battlepack, limit)
    ...
    if len(generals) == 0:
        ...
        if not _has_keyword(gen.unit, "CHARACTER"):
            issues.append(Issue(
                ...
                message=(
                    f'Warlord "{gen.unit.name}" não possui a palavra-chave CHARACTER.'
                ),
    ...
    if len(epic_hero_aus) > 3:
        issues.append(Issue(
            ...
            message=(
                f"{len(epic_hero_aus)} unidades EPIC_HERO no exército; máximo 3 permitido."
            ),
        ))
